...
//    In the scenarion above, we are declaring 2 variables. In one we are assiging an array with 4 elements and in the other the value 1.
...
// a) Your answer: it will log the weekendDays array with the string elements inside with all of its letters converted to large uppercase letters. ["SATURDAY", "SUNDAY"]
...
//    and apply the method to each element. To do this we need to iterate through the array using a forloop or we could also use the higher order functions .map() or .forEach().
...
//    have 3 elements so dataTypes.length will be equal to the number 3. So the console should log the string 'number'.
